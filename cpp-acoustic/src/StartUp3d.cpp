...
//used for project function c^2 on each element 
void AdaptiveM(Mesh *mesh){
  ...
  //generate the projected coefficient w.r.t Bernstein Bezier basis
  MatrixXd c_BB = invVB * Vandermonde3D(m,x,y,z)*VMq.transpose() * wq.asDiagonal() * mesh->Cq;

  //cout<<"V="<<endl<<Vandermonde3D(m,x,y,z)<<endl;
  //cout<<"CqM="<<endl<<CqM<<endl;
  //cout<<"c_BB="<<endl<<c_BB<<endl;
  // upload c_BB to mesh
...
void projection_nodal(Mesh *mesh){
  ...
  MatrixXd Cq(xq.rows(),xq.cols());
  for(int j=0; j < xq.cols(); ++j){
    for(int i=0; i < xq.rows(); ++i){
      // use 1/c^2 = 1+0.5*sin(2pix)*sin(2piy)*sin(2piz)
      Cq(i,j)= 1 + 0.5* sin(M_PI*xq(i,j))*sin(M_PI*yq(i,j))*sin(M_PI*zq(i,j));
    }
  }
  //upload Cq to mesh
  mesh->Cq = Cq;
...
void BB_mult(Mesh *mesh){
  ...
  VectorXd rq,sq,tq,wq;
  tet_cubature(3*N+1,rq,sq,tq,wq);
  ...
  //cout<<"L_id"<<endl<<L_ids<<endl;
  //cout<<"col_ids"<<endl<<col_ids<<endl;
  //cout<<"col_val"<<endl<<col_vals<<endl;
...
void BB_projection(Mesh *mesh){
  ...
  //cout<<"ENMT_index"<<endl<<ENMT_index<<endl;
