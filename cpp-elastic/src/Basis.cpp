...
void xytors(VectorXd x, VectorXd y,
            VectorXd &r, VectorXd &s){

  VectorXd I;
  I.setOnes(x.size());

  VectorXd L1 = (sqrt(3.0)*y+I)/3.0;
  VectorXd L2 = (-3.0*x-sqrt(3.0)*y + 2.0*I)/6.0;
  VectorXd L3 = ( 3.0*x-sqrt(3.0)*y + 2.0*I)/6.0;
...
void xyztorst(VectorXd x, VectorXd y, VectorXd z,
              VectorXd &r, VectorXd &s, VectorXd &t){

  VectorXd v1(3); v1 << -1, -1/sqrt(3), -1/sqrt(6);
  VectorXd v2(3); v2 <<  1, -1/sqrt(3), -1/sqrt(6);
  VectorXd v3(3); v3 <<  0,  2/sqrt(3), -1/sqrt(6);
  VectorXd v4(3); v4 <<  0,  0/sqrt(3),  3/sqrt(6);

  int length = x.size();
  MatrixXd rhs(3,length);
  rhs.row(0) = x.adjoint();
  rhs.row(1) = y.adjoint();
  rhs.row(2) = z.adjoint();
  ...
  MatrixXd v0(1,x.size());
  v0.fill(1);

  rhs.row(0) =  rhs.row(0)-0.5*v(0)*v0;
  rhs.row(1) =  rhs.row(1)-0.5*v(1)*v0;
  rhs.row(2) =  rhs.row(2)-0.5*v(2)*v0;
...
void EquiNodes3D(int N, VectorXd &x, VectorXd &y, VectorXd &z){
  int Np = (N+1)*(N+2)*(N+3)/6;
  x.resize(Np);
  x.fill(0.0);
  y.resize(Np);
...
float lami3D(int N,int i){
  float v = 12*fac(N,N-i);
  float w = fac(N+i+3,N);
  float v1 = v/w;
  return v1;
}

MatrixXd eig_ratio(int N, int M, int i){
  int c = N-i;
  MatrixXd V(c+1,1);
  V.fill(0.0);

  for(int j=0;j<c+1;++j){
    float w = lami3D(N-i,j);
    float w1 = lami3D(N+M,j);
    V(j,0) = w/w1;
  }
  return V;
}

MatrixXd eig_matrix(int N, int M){
  MatrixXd V(N+1,N+1);
  V.fill(0);

  for(int ii=0; ii<N+1; ++ii){
    MatrixXd lam = eig_ratio(N,M,ii);
    VectorXd W = V.col(ii);
    W.head(lam.size()) = lam;
    V.col(ii) = W;
  }
...
MatrixXd c_coefficient(int N, int M){
  MatrixXd L= eig_matrix(N,M);
  MatrixXd b(N+1,1);
  b.fill(1.0);
  MatrixXd c = mldivide(L,b);
  return c;
}
...
MatrixXd pinv(MatrixXd A){
  ...
  MatrixXd svd_U = svd.matrixV();
  MatrixXd svd_V = svd.matrixU();
  MatrixXd singular = svd.singularValues();
  
  MatrixXd singular_inv(singular.rows(),1);
  for(int i=0; i< singular.rows();++i){
    if(abs(singular(i,0))>1e-6){
      singular_inv(i,0)=1.0/singular(i,0);
      }else{
      singular_inv(i,0) = 0.f;
    }
  }
  
  
  MatrixXd B = svd_V*singular_inv.asDiagonal()*svd_U.transpose();
